...
parser.add_argument("--bamboo", help="Build for Bamboo", action="store_true")
...
n.rule("link", command="$cxx -o $out $in $libs $ldflags", description="LINK $out")

n.rule("install", command="ln -sf $absdir/$in $out", description="INSTALL $out")

n.rule("generate", command="./$in $args", description="GENERATE $out", restat=True)

# n.rule('rust',
#        command = 'cargo build $rflags -p $out',
#        description = 'CARGO $out')

n.rule("run", command="$in $args", description="RUN $out")

n.rule("configure", command="./$in", description="CONFIGURE $out", generator=True)
...
n.rule("general", command="./$in $args", description="$name $out", restat=True)
...
for d in dirs:
    n.build(
        ...
        "configure",
        "%s/configure.py" % d,
        implicit=["build/ninja_common.py"],
    )
