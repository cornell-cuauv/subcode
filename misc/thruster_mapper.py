...
def load_serial_conf(fname):
    ...
    if not os.path.isfile(full_path):
        raise Failure("File '{}' does not exist on path '{}'".format(fname, SERIAL_CONF_DIR))
    try:
        with open(full_path) as f:
            return tomlkit.parse(f.read())
    except Exception as e:
        raise Failure('Failure parsing {}'.format(fname), e)
...
def main():
    ...
    if not 'includes' in vehicle_conf or not isinstance(vehicle_conf['includes'], list):
        raise Failure("Serial configuration for {} is missing 'includes' field".format(VEHICLE))
    ...
    for include in vehicle_conf['includes']:
        ...
        motor_desires = []
        ramp_status = []
        find_values(board_conf, lambda s: s.startswith(MOTOR_DESIRES), type_filter=str, lst=motor_desires)
        find_values(board_conf, lambda s: s.startswith(RAMP_STATUS), type_filter=str, lst=ramp_status)

        if len(motor_desires) != len(ramp_status):
            raise Failure("Invalid thruster conf: '{}', incompatible {} and {}".format(include, MOTOR_DESIRES, RAMP_STATUS))

        ramp_status_map = {}
        for path, shm_var in ramp_status:
            # chop the ramp suffix at the end, i.e. the '_ramp'
            motor_desire_shm_var = '{}{}'.format(MOTOR_DESIRES, shm_var[len(RAMP_STATUS):-len(RAMP_SUFFIX)])
        ...
        for path, shm_var in motor_desires:
            if not shm_var in ramp_status_map:
                raise Failure("Invalid thruster conf: '{}', missing {} for {}"
                              .format(include, RAMP_STATUS, shm_var[len(MOTOR_DESIRES) + 1:]))
    ...
    if shm_names != conf_names:
        e = Failure('Mismatch between shm.{} and thrusters defined in serial conf!'.format(MOTOR_DESIRES))
        e.extra('Found thrusters in these files: {}'.format(list(thruster_files.keys())))
    ...
    if shm.switches.soft_kill.get():
        if input(colored('Soft kill enabled. Override? [yN] ', bcolors.WARNING)).strip().lower() in ['y', 'yes']:
    ...
    print('Enter the name of the thruster (or a uniquely-defining prefix) to assign that thruster.')
    ...
    for shm_name in sorted(shm_names):
        ...
        while not passed:
            ...
            if user_input == '':
                ...
                matches = set(filter(lambda name: name.startswith(user_input), shm_names))
                if len(matches) == 1:
                    match = tuple(matches)[0]
                    if match in already_mapped:
                        print(colored("Already mapped thruster '{}'".format(match), bcolors.WARNING))
                    else:
                        re_mapping[shm_name] = match
                        already_mapped.add(match)
                        passed = True
                elif len(matches) == 0:
                    print(colored("No thruster found for '{}'".format(user_input), bcolors.WARNING))
                else:
                    print(colored("Multiple matches found for '{}': {}".format(user_input, matches), bcolors.WARNING))

    for prev_name, (fname, motor_desires_path, ramp_status_path) in shm_map.items():
        chopped_prev_name = prev_name[len(MOTOR_DESIRES) + 1:]
        assert chopped_prev_name in re_mapping
        new_motor_desires_name = '{}.{}'.format(MOTOR_DESIRES, re_mapping[chopped_prev_name])
        new_ramp_status_name = '{}.{}{}'.format(RAMP_STATUS, re_mapping[chopped_prev_name], RAMP_SUFFIX)

        # Update motor desires
        deep_set(thruster_files[fname], motor_desires_path, new_motor_desires_name)
        # Update ramp status
    ...
    overwrite = input(colored('Overwrite previous thruster configuration? [yN] ',
    ...
    for fname, toml in thruster_files.items():
        ...
        if overwrite:
            ...
            print()
            print(colored("Proposed contents of '{}' [the file has not been changed]:".format(fname), bcolors.OKBLUE))
            print(output)

    if overwrite:
        print()
        print(colored('Saved. Restart serial daemon for changes to take effect.', bcolors.OKBLUE))
