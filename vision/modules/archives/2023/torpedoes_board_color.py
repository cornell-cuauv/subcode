...
module_options = [
    ...
    IntOption('min a', 130, 0, 255),
    IntOption('max a', 134, 0, 255),
    IntOption('min b', 125, 0, 255),
    IntOption('max b', 138, 0, 255),
    IntOption('kernel size', 20, 0, 35),
    DoubleOption('min rectangularity', 0.3, 0.2, 1.0),
    BoolOption('accept right triangles', True),
    IntOption('angle tolerance', 10, 0, 30)
    # IntOption('canny min', 167, 0, 255),
...
class TorpedoesBoard2023(ModuleBase):
    def process(self, img):
        ...
        thresh = (range_threshold(a, self.options['min a'], self.options['max a'])
                & range_threshold(b, self.options['min b'], self.options['max b']))
        ...
        # # Choose two vertical lines which sufficiently far apart
        ...
        # # Choose three horizontal lines which are sufficiently far apart and
        # # which have equal distance between them
        # chosen_horizontal_lines = None
        # for a in horizontal_lines:
        #     for b in horizontal_lines:
        #         for c in horizontal_lines:
        #             if a[1][0] - b[1][0] > 100 and b[1][0] - c[1][0] > 100:
        #                 if abs((a[1][0] - b[1][0]) - (b[1][0] - c[1][0])) < 30:
        ...
        #     draw_line(lines_img, (b[0][0], b[0][1]), (b[0][2], b[0][3]),
        #             color=(255, 255, 255), thickness=5)
        #     draw_line(lines_img, (c[0][0], c[0][1]), (c[0][2], c[0][3]),
        #             color=(255, 255, 255), thickness=5)
        ...
        #             chosen_horizontal_lines[2][0], chosen_vertical_lines[0][0])
        #     top_right_corner = line_intersection(
        #             chosen_horizontal_lines[2][0], chosen_vertical_lines[1][0])
