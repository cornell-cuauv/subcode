...
from math import pi, atan2, sin, cos, radians, degrees, dist
...
from vision.framework.feature import canny, simple_canny, find_circles, find_corners, find_lines, outer_contours, contour_area, contour_approx, contour_perimeter, find_line_segments
...
module_options = [
    ...
    IntOption('canny min', 75, 0, 255),
    IntOption('canny max', 255, 0, 255),
    DoubleOption('rho', 3, 0, 5),
    DoubleOption('theta', pi / 180, 0, pi / 10),
    DoubleOption('threshold', 125, 0, 500),
    IntOption('duplicate dist', 30, 0, 100),
...
class TorpedoesBoard2023(ModuleBase):
    def process(self, img):
        ...
        # Hough line segments
        lines = find_line_segments(canny_img, int(self.options['rho']), self.options['theta'], int(self.options['threshold']), max_line_gap=100)
        if lines is None:
            lines = []
        ...
        #         s0, e0 = (lines[i][0][0], lines[i][0][1]), (lines[i][0][2], lines[i][0][3])
        #         s1, e1 = (lines[j][0][0], lines[j][0][1]), (lines[j][0][2], lines[j][0][3])
        #         if dist(s0, s1) + dist(e0, e1) < dist(s0, e1) + dist(s1, e0):
        #             if dist(s0, s1) < self.options['duplicate dist'] and dist(e0, e1) < self.options['duplicate dist']:
        #                 duplicate = True
        #                 break
        #         else:
        #             if dist(s0, e1) < self.options['duplicate dist'] and dist(s1, e0) < self.options['duplicate dist']:
        #                 duplicate = True
        #                 break
        #     if not duplicate:
        #         fewer_lines.append(lines[i])
        # lines = fewer_lines
        ...
        for i, line in enumerate(lines):
            ...
            if (angle_diff(angle(line), 0) < horizontal_allowance
                    or angle_diff(angle(line), pi) < horizontal_allowance):
                horizontal_lines.append(lines[i])
            if (angle_diff(angle(line), pi / 2) < vertical_allowance
                    or angle_diff(angle(line), -pi / 2) < vertical_allowance):
        ...
        if (chosen_horizontal_lines is not None
            ...
            if not (bottom_left_corner is None or bottom_right_corner is None or top_left_corner is None or top_right_corner is None):
        ...
        if (chosen_horizontal_lines is not None
                and chosen_vertical_lines is not None
                and bottom_left_corner is not None
                and bottom_right_corner is not None
                and top_left_corner is not None
                and top_right_corner is not None):
            results.visible = True

            top = (top_left_corner[1] + top_right_corner[1]) / 2
            bottom = (bottom_left_corner[1] + bottom_right_corner[1]) / 2
            left = (bottom_left_corner[0] + top_left_corner[0]) / 2
            right = (bottom_right_corner[0] + top_right_corner[0]) / 2

            avg_x = (left + right) / 2
            avg_y = (top + bottom) / 2
            results.center_x, results.center_y = self.normalized((avg_x, avg_y))
            width = abs(right - left)
            height = abs(top - bottom)
            results.width, results.height = width / img.shape[1], height / img.shape[1]
            results.left_sidelength = abs((top_left_corner[1] - bottom_left_corner[1]) / img.shape[1])
            results.right_sidelength = abs((top_right_corner[1] - bottom_right_corner[1]) / img.shape[1])
        else:
            results.visible = True
        torpedoes_board_vision.set(results)
