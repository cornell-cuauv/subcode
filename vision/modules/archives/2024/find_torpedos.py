...
module_options = [
  ...
  options.IntOption('min_thresh', 52, 0, 255),
]
...
class DepthTorpedo(ModuleBase):
    def process(self, img):
        ...
        threshed_board = binary_threshold_inv(img, self.options['min_thresh'])
        self.post('orig', img)
        ...
        if board_result is not False:
            ...
            # goals
            threshed_goals = binary_threshold((img), self.options['min_thresh'])
            ...
            goals_circles = self.find_goals(goals_contours,tr,tl,br,bl)
    ...
    def find_goals(self,contours,tr,tl,br,bl):
        print('finding goals')
    
        max_num = 4
        pos_goals = []
        corners = [np.array(tr),np.array(tl),np.array(br),np.array(bl)]
        for i in range(len(contours)):
            cont = contours[i]
            (center, radius) = min_enclosing_circle(cont)
            center_np = np.array(center)
            if self.in_board(center_np,radius,corners):
                pos_goals.append((cont,center_np,np.pi*radius**2,radius))

        sorted_pos_goals = sorted(pos_goals, key=lambda x: x[2])
        circle_goals = []
        ...
        # print(g < max_num)
        # print()
        while (g < max_num) and (i < len(sorted_pos_goals)):
            if len(sorted_pos_goals) > i:
                g+=1
                visible = 1
            ...
            if visible == 1:
                ...
                getattr(shm, f"depth_goal_{g}").center_x.set(sorted_pos_goals[i][1][0])
                getattr(shm, f"depth_goal_{g}").center_y.set(sorted_pos_goals[i][1][1])
            ...
            to_print =  {
                'area': sorted_pos_goals[i][2],
                'center_x': sorted_pos_goals[i][1][0],
                'center_y': sorted_pos_goals[i][1][1],
                'confidence': 1,
                'visible': visible,
                'radius':sorted_pos_goals[i][3],
                'int_num':i+1
            }
    ...
    def find_board(self,img,contours):
        ...
        # set contour area or board area
        getattr(shm, f"depth_torpedos_board").area.set(area)
        getattr(shm, f"depth_torpedos_board").center_x.set(center_x)
        getattr(shm, f"depth_torpedos_board").center_y.set(center_y)
    ...
    def in_board(self,center,radius,corners):
        for c in range(4):
            origin = corners[c]
            to_goal = center - origin

            if c+1 < 4:
                right_edge = corners[c+1]-origin
            else:
                right_edge = corners[0]-origin
# 
            proj_vec = (np.dot(to_goal,right_edge)/ np.dot(right_edge,right_edge)) * right_edge
            perp_vec = to_goal-proj_vec

            perp_length = np.linalg.norm(perp_vec)

            if perp_length > radius:
                return False
