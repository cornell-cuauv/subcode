...
opts =    [options.DoubleOption('rectangular_thresh', 0.7, 0, 1),
           ...
           options.DoubleOption('source_x_scale_aswang', 0.65, 0, 1), #CHANGE BACK TO 1 IF COMPETITION BUOY IS NOT STRETCHED
           ...
           options.DoubleOption('downsize_camera', 1, 0, 1),
...
class VampBuoy(ModuleBase):
    def __init__(self, *args, **kwargs):
        ...
        #self.detector = cv2.ORB_create(nfeatures=250, WTA_K=3)
    ...
    def static_process(self, image):
        def find_key_descriptors(im):
            if self.options['source_x_scale_%s'%image] !=0 and self.options['source_y_scale_%s'%image] != 0:
                scaledim = resize(im, int(im.shape[1]*self.options['source_x_scale_%s'%image]), int(im.shape[0]*self.options['source_y_scale_%s'%image]))
            ...
            self.static[image] = {"name": image, "img": scaledim, "rx": rx, "ry":ry, "kp":kp, "des":des}
            self.post(image, cv2.drawKeypoints(scaledim, kp, None, (0,0,255), flags=0))
            return self.static[image]
        try:
            ret = self.static[image]
            if ret['rx'] == self.options['source_x_scale_%s'%image] and ret['ry'] == self.options['source_y_scale_%s'%image]: 
                return self.static[image]
            else: 
    ...
    def match(self, im1, im2, output, color):
        ...
        if len(good)>MIN_MATCH_COUNT:
            ...
            try:
                ...
                if area/rarea < RECTANGULARITY_THRESH: raise cv2.error("box lower than rectangularity threshold")
