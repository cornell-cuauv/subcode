...
class CameraTarget(Task):
    def on_run(self, point, target, deadband=(0.01875, 0.01875), px=None, ix=0, dx=0, py=None, iy=0, dy=0, max_out=None, valid=True, *args, **kwargs):
        ...
        if valid:
            ...
            target = call_if_function(target)
            self.pid_loop_x(input_value=point[0], p=px, i=ix, d=dx, target=target[0], deadband=deadband[0], max_out=max_out)
            self.pid_loop_y(input_value=point[1], p=py, i=iy, d=dy, target=target[1], deadband=deadband[1], max_out=max_out)
        else:
           self.stop()
