...
gate = Sequential(
    ...
    Log('Heading Searching for gate'),
    SearchFor(
        Sequential(
            # manual list of "check here first, then just StillHeadingSearch"
            FunctionTask(lambda: init_heading = shm.kalman.heading.get()),
    ...
    ConsistentTask(Concurrent(
        Depth(DEPTH_TARGET),
        Sequential(
            Log('Centering on (leftmost) leg of gate'),
            XTarget(x=shm.gate.leftmost_x, db=0.03)
        ),
        finite=False
    )),
    Log('Driving forward...'),
    MasterConcurrent(
        Consistent(
            test=lambda: results_groups.width.get() < settings.gate_width_threshold,
            count=0.2, total=0.3, invert=True, result=True
        ),
        Depth(DEPTH_TARGET),
        VelocityX(0.1 if is_mainsub() else 0.1),
        While(
            task_func=lambda: XTarget(x=results_groups.gate_center_x.get, db=0.018),
            condition=True
        ),
    ),
    # Jank
