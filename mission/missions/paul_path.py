...
#def AlignedMove(vector_getter, target_angle, dst_along):
#    return MasterConcurrent(
#            PidLoop(lambda: np.cross((vector_getter
...
class FirstPipeGroupFirst(Task):
    ...
    def on_run(self, bend_right):
        ...
        v1, v2 = np.complex128(np.exp([[angle_1], [angle_2]])).view(np.float64)
        #v2 = np.complex128([np.exp(angle_2)]).view(np.float64)
...
PipeAlign = lambda heading: Sequential(
    DownwardTarget(lambda: (shm.path_results.center_x.get(), shm.path_results.center_y.get()),
                   target=(0, -.25),
                   deadband=(.1, .1), px=0.5, py=0.5, ix=.01, iy=.01),
    Log("Centered on Pipe in PipeAlign!"),
    FunctionTask(lambda: shm.navigation_desires.heading.set(-180/math.pi*heading.get()+shm.kalman.heading.get()))
)
...
FollowPipe = lambda h1, h2: Sequential(
    ...
    FunctionTask(lambda: shm.navigation_desires.heading.set(-180/math.pi*h2.get()+shm.kalman.heading.get())),
    Timer(4),
